...
def setup_dss_repo():
    print("--- DSS CI/CD Project Initializer ---")
    ...
    try:
        ...
        if created:
            # Get the authenticated user's namespace to build the URL
        ...
        if os.path.exists(helper_path):
            print(f"> Injecting {project_id} into dss_workflow_helper.py...")
        ...
        # 6. Push to New Private Repo
        print("> Pushing finalized skeleton to remote...")
        ...
        run_command(f"git commit -m 'Initial setup for {project_id}'", cwd=local_dir)
        ...
        # 7. Final Cleanup
        if os.path.exists(local_dir):
            print(f"> Cleaning up temporary files in {local_dir}...")
            shutil.rmtree(local_dir)
